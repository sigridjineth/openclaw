...
describe("buildAgentSystemPrompt", () => {
  it("uses the Claude-CLI-safe OpenClaw identity line", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    });

    expect(prompt.startsWith("You are a personal assistant for an OpenClaw user.")).toBe(true);
    expect(prompt).not.toContain("You are a personal assistant running inside OpenClaw.");
  });

  it('returns only the safe identity line in promptMode="none"', () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      promptMode: "none",
    });

    expect(prompt).toBe("You are a personal assistant for an OpenClaw user.");
  });
