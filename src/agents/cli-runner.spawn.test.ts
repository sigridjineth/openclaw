...
describe("runCliAgent spawn path", () => {
  ...
  it("omits heartbeat guidance from Claude CLI system prompts", async () => {
    const runCliAgent = await setupCliRunnerTestModule();
    mockSuccessfulCliRun();
    ...
    await runCliAgent({
      ...
      config: {
        agents: {
          defaults: {
            heartbeat: {
              prompt: "Read HEARTBEAT.md if it exists. Reply HEARTBEAT_OK.",
            },
          },
        },
      ...
      model: "sonnet",
      timeoutMs: 1_000,
      runId: "run-no-heartbeat-claude",
    });

    const input = supervisorSpawnMock.mock.calls[0]?.[0] as { argv?: string[] };
    const allArgs = (input.argv ?? []).join("\n");
    expect(allArgs).not.toContain("HEARTBEAT_OK");
    expect(allArgs).not.toContain("Read HEARTBEAT.md");
  });
