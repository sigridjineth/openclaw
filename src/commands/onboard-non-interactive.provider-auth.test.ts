...
const CLI_PROXY_API_PROVIDER_ID = "cli-proxy-api";
const CLI_PROXY_API_DEFAULT_BASE_URL = "http://127.0.0.1:8317";
const CLI_PROXY_API_DEFAULT_MODEL_ID = "claude-sonnet-4-6";
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("configures CLI Proxy API auth choice with Anthropic defaults", async () => {
    await withOnboardEnv("openclaw-onboard-cli-proxy-api-", async ({ configPath, runtime }) => {
      await runNonInteractiveSetupWithDefaults(runtime, {
        authChoice: "cli-proxy-api",
        customApiKey: "proxy-test-key", // pragma: allowlist secret
        skipSkills: true,
      });

      const cfg = await readJsonFile<ProviderAuthConfigSnapshot>(configPath);
      const provider = cfg.models?.providers?.[CLI_PROXY_API_PROVIDER_ID];

      expect(provider?.baseUrl).toBe(CLI_PROXY_API_DEFAULT_BASE_URL);
      expect(provider?.api).toBe("anthropic-messages");
      expect(provider?.apiKey).toBe("proxy-test-key");
      expect(provider?.models?.some((model) => model.id === CLI_PROXY_API_DEFAULT_MODEL_ID)).toBe(
        true,
      );
      expect(cfg.agents?.defaults?.model?.primary).toBe(
        `${CLI_PROXY_API_PROVIDER_ID}/${CLI_PROXY_API_DEFAULT_MODEL_ID}`,
      );
    });
  });

  it("stores CLI_PROXY_API_KEY env ref for CLI Proxy API in ref mode", async () => {
    await withOnboardEnv("openclaw-onboard-cli-proxy-api-ref-", async ({ configPath, runtime }) => {
      process.env.CLI_PROXY_API_KEY = "proxy-env-key"; // pragma: allowlist secret
      await runNonInteractiveSetupWithDefaults(runtime, {
        authChoice: "cli-proxy-api",
        secretInputMode: "ref", // pragma: allowlist secret
        skipSkills: true,
      ...
      const cfg = await readJsonFile<ProviderAuthConfigSnapshot>(configPath);
      expect(cfg.models?.providers?.[CLI_PROXY_API_PROVIDER_ID]?.apiKey).toEqual({
        source: "env",
        provider: "default",
  ...
  it("rejects non-Anthropic compatibility overrides for CLI Proxy API", async () => {
    await withOnboardEnv("openclaw-onboard-cli-proxy-api-invalid-compat-", async ({ runtime }) => {
      await expect(
