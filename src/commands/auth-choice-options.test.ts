...
describe("buildAuthChoiceOptions", () => {
  ...
  it("builds cli help choices from the same runtime catalog", () => {
    ...
    expect(cliChoices).toContain("cli-proxy-api");
  ...
  it("shows CLI Proxy API in grouped provider selection", () => {
    resolveManifestProviderAuthChoices.mockReturnValue([]);
    const { groups } = buildAuthChoiceGroups({
      store: EMPTY_STORE,
      includeSkip: false,
    });
    const proxyGroup = groups.find((group) => group.value === "cli-proxy-api");

    expect(proxyGroup).toBeDefined();
    expect(proxyGroup?.options.some((opt) => opt.value === "cli-proxy-api")).toBe(true);
  });
