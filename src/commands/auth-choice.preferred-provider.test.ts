...
describe("resolvePreferredProviderForAuthChoice", () => {
  ...
  it("maps CLI Proxy API to its built-in provider id", async () => {
    await expect(resolvePreferredProviderForAuthChoice({ choice: "cli-proxy-api" })).resolves.toBe(
      "cli-proxy-api",
    );
    expect(resolvePluginProviders).toHaveBeenCalled();
  });
});
