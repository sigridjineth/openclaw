...
import {
  ...
  CLI_PROXY_API_KEY_ENV_VAR,
  CustomApiError,
  parseNonInteractiveCliProxyApiFlags,
...
export async function applyNonInteractiveAuthChoice(params: {
  ...
  if (authChoice === "custom-api-key" || authChoice === "cli-proxy-api") {
    try {
      const customAuth =
      ...
      const envVarName =
        authChoice === "cli-proxy-api" ? CLI_PROXY_API_KEY_ENV_VAR : "CUSTOM_API_KEY";
      ...
      if (result.providerIdRenamedFrom && result.providerId) {
        runtime.log(
          `${
            authChoice === "cli-proxy-api" ? "CLI Proxy API" : "Custom provider"
          } ID "${result.providerIdRenamedFrom}" already exists for a different base URL. Using "${result.providerId}".`,
        );
      }
