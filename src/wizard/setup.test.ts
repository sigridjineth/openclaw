...
const promptCliProxyApiConfig = vi.hoisted(() => vi.fn(async (args) => ({ config: args.config })));
...
describe("runSetupWizard", () => {
  ...
  it("routes CLI Proxy API auth through the dedicated prompt and skips the model picker", async () => {
    promptCliProxyApiConfig.mockClear();
    promptDefaultModel.mockClear();
    ...
    expect(promptCliProxyApiConfig).toHaveBeenCalled();
