...
export async function runSetupWizard(
  ...
  const { promptCliProxyApiConfig, promptCustomApiConfig } =
  ...
  if (authChoice === "custom-api-key") {
    ...
    const proxyResult = await promptCliProxyApiConfig({
      prompter,
      runtime,
      config: nextConfig,
      secretInputMode: opts.secretInputMode,
    });
    nextConfig = proxyResult.config;
  } else {
  ...
  const authChoiceModelSelectionPolicy =
    authChoice === "custom-api-key" || authChoice === "cli-proxy-api"
      ? undefined
