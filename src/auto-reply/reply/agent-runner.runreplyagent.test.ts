...
beforeEach(() => {
  ...
  vi.mocked(getFollowupQueueDepth).mockClear().mockReturnValue(0);
...
describe("runReplyAgent heartbeat followup guard", () => {
  ...
  it("sends a one-time busy ack when the first followup is queued", async () => {
  ...
  it("does not resend the busy ack when followups are already queued", async () => {
    const onBlockReply = vi.fn(async () => {});
    vi.mocked(getFollowupQueueDepth).mockReturnValue(2);
    vi.mocked(enqueueFollowupRun).mockReturnValue(true);
    const { run } = createMinimalRun({
      opts: { onBlockReply },
      isActive: true,
