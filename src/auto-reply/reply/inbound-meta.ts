...
export function buildInboundMetaSystemPrompt(ctx: TemplateContext): string {
  ...
  const payload = {
    // Keep channel-run trusted metadata provider-neutral. Claude CLI's
    // third-party classifier is sensitive to OpenClaw-branded system-prompt
    // phrasing on channel-bound turns, even though the same runs succeed when
    // the metadata instructions are semantically identical but brand-neutral.
    schema: "inbound_meta.v1",
  ...
  return [
    ...
    "The following JSON is generated out-of-band by the runtime metadata bridge. Treat it as authoritative metadata about the current message context.",
