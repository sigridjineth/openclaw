...
function resolveBundledPluginManifestRecordByDirNameWithoutConfig(
  dirName: string,
): PluginManifestRecord | null {
  return (
    loadPluginManifestRegistry({
      config: EMPTY_FACADE_BOUNDARY_CONFIG,
      cache: true,
    }).plugins.find(
      (plugin) => plugin.origin === "bundled" && path.basename(plugin.rootDir) === dirName,
    ) ?? null
...
function resolveTrackedFacadePluginId(dirName: string): string {
  return resolveBundledPluginManifestRecordByDirNameWithoutConfig(dirName)?.id ?? dirName;
}
