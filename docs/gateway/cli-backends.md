...
read_when:
  ...
  - You are running Codex CLI, Claude CLI, or other local AI CLIs and want to reuse them
...
Claude CLI also works out of the box (via the bundled Anthropic plugin):
...
The bundled OpenAI plugin registers a default for `codex-cli`:
...
The bundled Anthropic plugin also registers a default for `claude-cli`:
