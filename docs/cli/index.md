...
- `--auth-choice <choice>` where `<choice>` is one of:
  ...
  `modelstudio-api-key-cn`, `modelstudio-api-key`, `custom-api-key`, `cli-proxy-api`, `skip`
...
- `--custom-base-url <url>` (non-interactive; used with `--auth-choice custom-api-key` or `cli-proxy-api`)
- `--custom-model-id <id>` (non-interactive; used with `--auth-choice custom-api-key` or `cli-proxy-api`)
- `--custom-api-key <key>` (non-interactive; optional; used with `--auth-choice custom-api-key` or `cli-proxy-api`; falls back to `CUSTOM_API_KEY` or `CLI_PROXY_API_KEY` depending on auth choice)
- `--custom-provider-id <id>` (non-interactive; optional custom provider id)
- `--custom-compatibility <openai|anthropic>` (non-interactive; optional; default `openai`; `cli-proxy-api` requires `anthropic`)
- `--gateway-port <port>`
