...
<AccordionGroup>
  ...
  <Accordion title="CLI Proxy API">
    Connects to a local Claude Code OAuth bridge using Anthropic-compatible HTTP.

    Non-interactive flags:
    - `--auth-choice cli-proxy-api`
    - `--custom-api-key` (optional; falls back to `CLI_PROXY_API_KEY`)
    - `--custom-base-url` (optional; default `http://127.0.0.1:8317`)
    - `--custom-model-id` (optional; default `claude-sonnet-4-6`)
    - `--custom-provider-id` (optional; default `cli-proxy-api`)

    Use the base URL without `/v1`. More detail: [CLI Proxy API](/providers/cli-proxy-api).
