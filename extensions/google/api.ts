...
function applyAgentDefaultModelPrimaryLocally(
  cfg: OpenClawConfig,
  primary: string,
): OpenClawConfig {
  ...
  const existingFallbacks =
    currentModel && typeof currentModel === "object" && "fallbacks" in currentModel
      ? Array.isArray(currentModel.fallbacks)
        ? currentModel.fallbacks.map((value) => String(value))
        : undefined
      : undefined;
  return {
    ...cfg,
...
export function applyGoogleGeminiModelDefault(cfg: OpenClawConfig): {
  ...
  return {
    next: applyAgentDefaultModelPrimaryLocally(cfg, GOOGLE_GEMINI_DEFAULT_MODEL),
    changed: true,
  };
}
